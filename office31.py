...
def load_data(source_list, target_list, batch_size, num_workers, noisy_rate, noise_type,task = 'cross'):
    office31.init(source_list, target_list, task)
    query_dataset = office31(noise_type,'query', query_transform(),target_transform=Onehot(),aug_transform=train_aug_transform(), noisy_rate = noisy_rate)
    train_s_dataset = office31(noise_type,'train_s', train_transform(),target_transform=Onehot(),aug_transform=train_aug_transform(),noisy_rate = noisy_rate)
    train_t_dataset = office31(noise_type,'train_t', train_transform(),target_transform=Onehot(),aug_transform=train_aug_transform(),noisy_rate = noisy_rate)
    retrieval_dataset = office31(noise_type,'retrieval', query_transform(), target_transform=Onehot(),aug_transform=train_aug_transform(),noisy_rate = noisy_rate)
...
class office31(Dataset):
    def __init__(self, noise_type,mode, transform=None, target_transform=None, aug_transform=None, noisy_rate=0.2, noise = True):
        ...
        if mode == 'train_s':
            ...
            if noise:
                ...
                self.targets, self.actual_noise_rate = noisify_dataset(31, train_labels, noise_type, noisy_rate, 0, 0, True)
    ...
    def __getitem__(self, index):
        ...
        return img_t, img_aug, self.target_transform(self.targets[index]), index
    ...
    def init(source_list, target_list, task):
        ...
        with open(source_list, 'r') as f:
            for line in f.readlines():
                ...
                source_data.append(line.split()[0].replace(\
                    '/data/office/domain_adaptation_images','/hdd/DataSet/OFFICE31'))
        ...
        if task == 'cross':
            ...
            logger.info('Query Num: {}'.format(office31.QUERY_DATA.shape[0]))
            logger.info('Retrieval Num: {}'.format(office31.RETRIEVAL_DATA.shape[0]))
            logger.info('Source Train Num: {}'.format(office31.TRAIN_S_DATA.shape[0]))
            logger.info('Target Train Num: {}'.format(office31.TRAIN_T_DATA.shape[0]))
